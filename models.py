...
class HomogeneousUnravelingModel(ContagionModel):
    def __init__(self,params):
        super(HomogeneousUnravelingModel, self).__init__(params)
    ...
    def set_activation_functions(self):
        ...
        # Set the threshold of unraveling
    ...
    def loop_over_exposed_nodes(self, current_network):
        ...
        for i in self.list_of_exposed_agents:
            ...
            # Find the number of neighbors that are not being infected (removed from the network)
            num_neighbor = 0
            for n in self.params['network'].neighbors(i):
                if self.params['network'].node[n]['state'] != infected * active and self.params['network'].node[n]['state'] != infected * inactive:
                    num_neighbor += 1

            if num_neighbor < self.params['threshold']:
        ...
        if self.updated_list_of_exposed_agents == self.list_of_exposed_agents:
            self.stable = True
        return
# 现在做到这里
...
class HeterogeneousUnravelingModel(ContagionModel):
    def __init__(self, params):
        super(HeterogeneousUnravelingModel, self).__init__(params)
        self.classification_label = SIMPLE  # or CMOPLEPX
        # set other model specific flags and handles here
    ...
    def set_activation_functions(self):
        ...
        # Set the threshold of unraveling (Homogeneous)
        # self.params['threshold'] = threshold
        random.seed(100)
        #threshold_set=np.random.normal(loc=50,scale=15,size=self.params['network'].number_of_nodes())
        # half-t: mu is the min, first para is df, second is loc, third is sigma(var)
        threshold_set = mu + np.abs(scipy.stats.t.rvs(1, 0, 1, self.params['network'].number_of_nodes()))
        for i in range(len(threshold_set)):
            if threshold_set[i]<0:
                threshold_set[i]=0
        # Round
        threshold_set = [round(thresholds) for thresholds in threshold_set]
        # Heterogeneous
        for i in self.params['network'].nodes():
            self.params['network'].node[i]['threshold']=threshold_set[int(i)]
    ...
    def loop_over_exposed_nodes(self, current_network):
        ...
        for i in self.list_of_exposed_agents:
            ...
            num_neighbor = 0
            for n in self.params['network'].neighbors(i):
                if self.params['network'].node[n]['state'] != infected * active and self.params['network'].node[n][
                    'state'] != infected * inactive:
                    num_neighbor += 1

            if num_neighbor < self.params['network'].node[i]['threshold']:
